...
from varoascii import __main__
...
def _write_demo_plugin(tmp_path: Path) -> None:
    ...
    plugin_dir = tmp_path / "varoascii" / "effects"
...
from varoascii.engine.base_config import BaseConfig
...
def test_bash_completion_registers_in_clean_shell() -> None:
    ...
    result = _run_bash(
        'eval "$('
        f"{sys.executable} -m varoascii --print-completion bash"
        ')"; complete -p tte; complete -p varoascii',
    )

    assert "complete -F _tte_completion tte" in result.stdout
    assert "complete -F _tte_completion varoascii" in result.stdout
...
def test_zsh_completion_registers_in_clean_shell() -> None:
    ...
    result = _run_zsh(
        'eval "$('
        f"{sys.executable} -m varoascii --print-completion zsh"
        ')"; whence -w _tte_completion; complete -p tte; complete -p varoascii',
    )
...
def test_bash_completion_suggests_effect_names_and_options() -> None:
    ...
    result = _run_bash(
        """
eval "$(""" + f"{sys.executable}" + """ -m varoascii --print-completion bash)"
COMP_WORDS=(tte ma)
COMP_CWORD=1
_tte_completion
printf 'effects:%s\\n' "${COMPREPLY[*]}"
...
def test_bash_completion_suggests_choice_and_file_values(tmp_path: Path) -> None:
    ...
    result = _run_bash(
        f"""
eval "$({sys.executable} -m varoascii --print-completion bash)"
