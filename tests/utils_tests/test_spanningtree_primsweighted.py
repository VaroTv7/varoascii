...
from varoascii.utils.spanningtree.algo.primsweighted import PrimsWeighted, WeightedLink

if TYPE_CHECKING:
    from varoascii.engine.base_character import EffectCharacter
...
def test_prims_weighted_init_selects_random_starting_character_when_not_provided(
    ...
    monkeypatch.setattr(
        "varoascii.utils.spanningtree.algo.primsweighted.random.randint",
        fake_randint,
    )
...
def test_get_lowest_weight_link_skips_stale_links_and_returns_next_lowest_valid_link(
    ...
    monkeypatch.setattr(
        "varoascii.utils.spanningtree.algo.primsweighted.random.randrange",
        fake_randrange,
    )
