...
* Added effect discovery support for user provided effects. Effects should be Python files located at
  XDG_CONFIG_HOME/varoascii/effects. (e.g. /home/me/.config/varoascii/effects/effect_custom.py)
...
* Generated shell completions now include user-provided effects discovered from `XDG_CONFIG_HOME/varoascii/effects` at completion-generation time.
...
* Updated the `varoascii.__main__` entry-point documentation to describe effect discovery, plugin loading, duplicate-command validation, and the CLI's input and exit behavior more accurately.
...
* `CharacterGroup`, `CharacterSort`, and `ColorSort` themselves were relocated from the `Terminal` module into `varoascii.utils.argutils`, and the terminal now imports them from there so both the CLI and the engine share a single definition of the enums.
* `varoascii.utils.argutils` introduces dedicated argument-type helpers for `CharacterGroup`, `CharacterSort`, and `ColorSort`.
...
* All effects are up-imported into `varoascii.effects` to simplify importing to `from varoascii.effects import Burn`.
...
* Geometry related methods have been removed from the motion class. They are now located at varoascii.utils.geometry as separate functions.
